...
public sealed class RasterizedGlyphTests
{
    ...
    public void Constructor_PreservesMetricsAndCoverage()
    {
        ...
        Assert.Equal(3, glyph.TopOffsetPixels);
        Assert.Equal(7, glyph.AdvancePixels);
        Assert.Equal(coverage, glyph.Coverage.ToArray());
    }

    [Fact]
    public void Constructor_PreservesSignedMetrics()
    {
        RasterizedGlyph glyph = new(
            widthPixels: 0,
            heightPixels: 0,
        ...
        Assert.Equal(-4, glyph.BearingLeftPixels);
        Assert.Equal(-2, glyph.TopOffsetPixels);
        Assert.Equal(-1, glyph.AdvancePixels);
    }
