...
public sealed class LauncherApplicationTests
{
    ...
    public async Task StopAsync_DrainsReentrantCallbacksBeforeCompleting()
    {
        ...
        TaskCompletionSource releaseEvaluation = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );

        TaskCompletionSource<Task> reentered = new(
            TaskCreationOptions.RunContinuationsAsynchronously
        );
        ...
        application = new LauncherApplication(
            new StubResolver(async token =>
            {
                ...
                return new ManagedInstallationResolution.Rejected(
                    ManagedInstallationIssue.ReadFailure
                );
            })
        );
        ...
        try
        {
            Task callbackShutdown = await reentered.Task.WaitAsync(
    ...
    public async Task StopAsync_PreservesCallbackAndEvaluationFailures()
    {
        ...
        try
        {
            ...
            Assert.Equal(
                new Exception[] { callbackFailure, evaluationFailure }.ToHashSet(),
    ...
    public async Task RetryInstallationAsync_RecoversWithoutRestartingTheLauncher()
    {
        ...
        ManagedInstallation installation = CreateManagedInstallation(root);

        int calls = 0;

        await using LauncherApplication application = new(
            new StubResolver(_ =>
                Task.FromResult<ManagedInstallationResolution>(
                    ++calls == 1
        ...
        Assert.Same(
            installation,
            Assert.IsType<LauncherState.InstallationResolved>(application.State).Installation
        );
    ...
    public async Task RetryInstallationAsync_UnexpectedFaultIsNotExposedAsRecoverable()
    {
        ...
        Assert.Same(
            failure,
            await Assert.ThrowsAsync<InvalidOperationException>(application.StopAsync)
        );
    ...
    {
        LauncherApplication application = new(
            new StubResolver(_ => throw new InvalidOperationException("Resolver must not run."))
        );
    ...
    {
        LauncherApplication application = new(
            new StubResolver(token =>
                Task.FromException<ManagedInstallationResolution>(
    ...
    public void ManagedInstallationRejectsPathsOutsideItsProductRoot()
    {
        ...
        string outsideClient = Path.Combine(Path.GetDirectoryName(root)!, "outside-client");
        ...
        Assert.Throws<ArgumentException>(() =>
            ManagedInstallation.Create(
                ...
                Path.Combine(outsideClient, "OpenConquer.Client"),
    ...
    private static ManagedInstallation CreateManagedInstallation(string rootPath)
    {
        ...
        return ManagedInstallation.Create(
            ...
            Path.Combine(root, ManagedReleaseManifest.FileName),
            Path.Combine(root, ManagedReleaseSignature.FileName),
            Path.Combine(clientRoot, clientExecutable),
            CreateReleaseIdentity(targetRuntime)
        );
    }

    private static ManagedReleaseIdentity CreateReleaseIdentity(string? targetRuntime = null)
    {
        return new ManagedReleaseIdentity(
            Sequence: 1,
            Version: "test-release",
            TargetRuntime: targetRuntime ?? ReleaseTargetRuntime.Current ?? "linux-x64"
        );
    }
