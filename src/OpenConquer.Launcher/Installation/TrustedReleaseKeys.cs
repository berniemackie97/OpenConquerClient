...
internal sealed class TrustedReleaseKeys
{
    ...
    public bool Verify(ManagedReleaseSignature signature, ReadOnlySpan<byte> data)
    {
        ...
        try
        {
            ...
            return bytesRead == publicKey.Length && algorithm.VerifyData(data, signature.Signature,
    ...
    public static bool IsValidKeyId(string? keyId)
    {
        ...
        return keyId.AsSpan(KeyIdPrefix.Length).All(character => character is >= '0' and <= '9' or >= 'a' and <= 'f');
    }

    internal static bool TryParse(ReadOnlySpan<byte> json, out TrustedReleaseKeys? keys)
    {
        keys = null;
        try
        {
            ...
            if (root.ValueKind != JsonValueKind.Object || root.EnumerateObject().Count() != 2 ||
                ...
                schemaVersion != CurrentSchemaVersion || !root.TryGetProperty("keys", out JsonElement entries) ||
            ...
            {
                if (publicKeys.Count == MaximumKeyCount || entry.ValueKind != JsonValueKind.Object ||
                    entry.EnumerateObject().Count() != 2 || !entry.TryGetProperty("algorithm", out JsonElement algorithm) ||
                    !entry.TryGetProperty("publicKey", out JsonElement encoded) || algorithm.ValueKind != JsonValueKind.String ||
                    encoded.ValueKind != JsonValueKind.String || !string.Equals(algorithm.GetString(), ManagedReleaseSignature.Algorithm, StringComparison.Ordinal))
                {
                    return false;
                }
    ...
    private static byte[] ValidatePublicKey(ReadOnlySpan<byte> subjectPublicKey)
    {
        ...
        if (bytesRead != subjectPublicKey.Length || parameters.Curve.Oid.Value != ECCurve.NamedCurves.nistP256.Oid.Value ||
