...
path in project metadata. A complete generated source manifest is a deliverable of the first import
slice.
...
The directory name is misleading: `ini/` is a mixed database containing text configuration,
compiled tables, binary records, region data, and terrain-effect payloads.
...
- Each of the 14 `.dbc` files has a same-stem text `.ini` counterpart. The DBC files are binary,
  not renamed text. Which representation wins is **unknown** until the native loader is verified.
- Eleven `.tme` payloads are duplicated byte-for-byte between `ini/tme/` and
  `ini/TerrainMagic/`. Their path identities must be preserved even if a compiled package later
...
- High-byte text appears throughout the configuration corpus. Much of it is consistent with a
  ...
  UTF-8 policy would corrupt some content. Encoding becomes explicit per format or per catalog
...
### Immediate compatibility discrepancy
...
The pre-import `GameSetupConfiguration` implementation and its tests expected:
...
This discrepancy is corrected in the first import slice. The typed reader and its tests now use the
...
The destination names are stable logical domains. They are not instructions to rename retail files
in place or to rewrite references by hand.
...
The import catalog therefore preserves every logical path. A later content-addressed payload store
may deduplicate bytes without collapsing asset identities.

### Legacy-only content

- The 28 `data/Cursor/*.ani` files are RIFF animated cursor binaries. They are unrelated to the
  text animation catalogs under top-level `ani/` despite sharing an extension.
...
- The 137 SWF files depend on retired Flash behavior. The default plan is to preserve them in source
  inventory but rebuild required flows in native OpenConquer UI. Embedding a Flash runtime requires
  a separate, explicit architecture and security decision.
...
or resource IDs to frames under `data/`. Other catalogs bind map imagery. This area is in the first
slice because moving images without their identity catalogs would create an unusable asset pile.

### WDF packages and loose overlays

`ini/package.ini` lists:
...
The first two packages exist; `data3.wdf` does not exist in the supplied snapshot. The existing
packages begin with the `PFDW` signature. Their headers declare 14,739 and 10,274 entries,
respectively, but those counts are not yet validated by a project reader.

A lexical scan of text catalogs found tens of thousands of unique `data/` and `c3/` references that
are not satisfied by the same relative path in the loose tree. Some formats also apply their own
path namespace or prefix. This proves that a loose-directory-only importer cannot establish the
complete dependency graph. Native package ordering, path hashing, prefix rules, and loose-file
precedence are **unknown** and must be verified before import selection.

The absent `data3.wdf` may be optional, stale configuration, or missing source. Import validation
must report it distinctly and must not decide its severity without native evidence.

### Deferred areas

- `map/` holds DMap terrain containers and related scene data. `ini/GameMap.dat` is its primary
  index. A verified reader already exists in the sibling server codebase and can be used as evidence,
  but client ownership and rendering composition remain separate work.
...
These areas are dependencies of the first catalog slice, not candidates for bulk migration in this
branch.
...
3. Logical organization belongs in a reviewed catalog; physical payload storage can remain
   path-preserving at ingestion and become content-addressed at packaging.
4. Extension alone is not a safe format discriminator.
5. WDF support and native precedence evidence are prerequisites for a complete import graph.
...
7. Asset-family migration is complete at the source-preservation boundary: every `ini/`, `data/`,
   and directly indexing `ani/` file is retained and cataloged. Runtime staging remains
   consumer-driven so unsupported formats do not enter a release merely because their source bytes
   are preserved.
