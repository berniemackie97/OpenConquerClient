...
/// <summary>
/// Verifies that a content set on disk is exactly what its manifest declares.
/// </summary>
/// <remarks>
/// The check is bidirectional. Every manifest entry must be present with the declared length,
/// SHA-256, and signature, and every payload file must be declared. A one-directional check would
/// pass a set that had silently lost files, which is precisely the failure this guards against.
/// </remarks>
internal static class ContentSetVerifier
{
    ...
    public static ContentManifest Verify(string contentSetRootPath)
    {
        ...
        Dictionary<string, ContentManifestEntry> expectedBySourcePath = manifest.Entries.ToDictionary(
        ...
        string[] missingSourcePaths = expectedBySourcePath.Keys
            .Where(sourcePath => !observedSourcePaths.Contains(sourcePath))
        ...
        if (missingSourcePaths.Length > 0)
        {
            throw new InvalidDataException(
                $"The content set is missing {missingSourcePaths.Length} declared payload file(s): {string.Join(", ", missingSourcePaths)}."
            );
        }
    ...
    private static ContentManifest ReadManifest(string manifestPath)
    {
        ...
        {
            throw new InvalidDataException(
                $"The content-set manifest is {manifestFile.Length} bytes; the limit is {ContentManifestReader.MaximumLength} bytes."
            );
        }
        ...
        HashSet<string> observedSourcePaths)
    {
        ...
        {
            throw new InvalidDataException(
                $"Content-set payload '{sourcePath}' is {file.Length} bytes; the manifest declares {expected.Length}."
            );
        }
        ...
        {
            throw new InvalidDataException(
                $"Content-set payload '{sourcePath}' has signature '{observedSignature}'; the manifest declares '{expected.Signature}'."
            );
        }
