...
internal sealed record ProductReleaseManifest(
    ...
    IReadOnlyList<ProductReleaseFile> Files)
{
    ...
    public static void Create(ReleaseManifestOptions options)
    {
        ...
        if (!ProductTargetRuntime.IsSupported(options.TargetRuntime) || !IsValidVersion(options.ReleaseVersion) ||
            options.ReleaseSequence == 0 || options.MinimumLauncherVersion <= 0)
        {
        ...
        IReadOnlyList<ProductReleaseFile> files = ReadClientFiles(clientRoot);
    ...
    public static void VerifyClient(string clientRoot, ProductReleaseManifest manifest)
    {
        ...
        IReadOnlyList<ProductReleaseFile> actual = ReadClientFiles(clientRoot);
        ...
        for (int index = 0; index < actual.Count; index++)
        {
            ...
            if (!string.Equals(expectedFile.Path, actualFile.Path, StringComparison.Ordinal) ||
                expectedFile.Length != actualFile.Length ||
                !CryptographicOperations.FixedTimeEquals(expectedFile.Sha256, actualFile.Sha256))
            {
                throw new InvalidDataException("The client publish does not match the release manifest.");
            }
        }
    ...
    private static IReadOnlyList<ProductReleaseFile> ReadClientFiles(string clientRoot)
    {
    ...
    private static void ReadDirectory(DirectoryInfo directory, string clientRoot, List<ProductReleaseFile> files,
        HashSet<string> portablePaths, CopyTraversalGuard traversal)
    {
        ...
        foreach (FileSystemInfo entry in directory.EnumerateFileSystemInfos().OrderBy(item => item.Name, StringComparer.Ordinal))
        {
            ...
            if (entry is not FileInfo file || (entry.Attributes & (FileAttributes.Directory | FileAttributes.Device)) != 0 ||
            ...
            if (stream.Length != length || File.GetAttributes(file.FullName).HasFlag(FileAttributes.ReparsePoint))
            {
    ...
    private static bool TryRead(JsonElement root, out ProductReleaseManifest? manifest)
    {
        ...
        if (schemaVersion != CurrentSchemaVersion || releaseSequence is null or 0 || minimumLauncherVersion is null or <= 0 ||
            !string.Equals(productId, ExpectedProductId, StringComparison.Ordinal) || !IsValidVersion(releaseVersion) ||
            !ProductTargetRuntime.IsSupported(targetRuntime) || !ProductReleasePath.IsValid(clientExecutable) || files is not { Count: > 0 } ||
            !files.Any(file => string.Equals(file.Path, clientExecutable, StringComparison.Ordinal)))
        {
            return false;
        }

        manifest = new ProductReleaseManifest(schemaVersion.Value, productId, releaseSequence.Value, releaseVersion!,
            minimumLauncherVersion.Value, targetRuntime!, clientExecutable!, files);
        return true;
    }
    ...
    private static bool TryReadFile(JsonElement item, out ProductReleaseFile? file)
    {
        ...
        if (!ProductReleasePath.IsValid(path) || length is null or < 0 || hash is not { Length: 64 } ||
    ...
    {
        return version is { Length: > 0 and <= MaximumVersionLength } && char.IsAsciiLetterOrDigit(version[0]) &&
            char.IsAsciiLetterOrDigit(version[^1]) && version.All(character => char.IsAsciiLetterOrDigit(character) || character is '.' or '-');
    }
